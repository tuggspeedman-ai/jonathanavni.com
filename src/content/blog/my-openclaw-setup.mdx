...
date: "2026-02-28"
excerpt: "How I set up OpenClaw as a personal AI assistant on a $20/month VPS — with automated newsletter triage, memory consolidation, and model routing for cost control."
...
**Memory consolidation (nightly):** Reads the past week of daily logs, distills significant events, updates `MEMORY.md`. Appends changelog to `memory/consolidation-log.md`.
...
**VPS isolation:** Process-level isolation with a controlled network perimeter. The gateway listens only on loopback and Tailscale—no public ports.
...
**Isolated cron sessions enable reliable automation.** Background tasks run separately with `delivery: none`, keeping the main thread clean.
...
**The newsletter pipeline is high-value automation.** This one workflow justified the setup. I went from ignoring a lot of these newsletters to reviewing synthesized takeaways weekly.

**Memory consolidation works.** Daily logs are raw; `MEMORY.md` is curated. The nightly job distills logs into durable context.
